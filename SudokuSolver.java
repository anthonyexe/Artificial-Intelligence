...
public class SudokuSolver {
	...
	public void availabilityCalc3D() {
		ArrayList<ArrayList<ArrayList<Integer>>> availabilityLists = new ArrayList<ArrayList<ArrayList<Integer>>>(81);
		...
		for (int i = 0, count = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				...
				if (puzzle[i][j] == 0) {
					...
					for (int k = 0; k < 9; k++) {
						if (puzzle[k][j] != 0) {
							availabilityLists.get(count).get(0).remove(puzzle[k][j]);
						}
						if (puzzle[i][k] != 0) {
							availabilityLists.get(count).get(0).remove(puzzle[i][k]);
						}
					}
					if (i <= 2 && j <= 2) {
						for (int r = 0; r <= 2; r++) {
							for (int c = 0; c <= 2; c++) {
								if (puzzle[r][c] != 0)
									availabilityLists.get(count).get(0).remove(puzzle[r][c]);
							}
						}
